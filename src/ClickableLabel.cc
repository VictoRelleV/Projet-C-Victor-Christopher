...
ClickableLabel::ClickableLabel(const QString& playerName, int attack, int defense, int speed, int position, QWidget& parent, QLabel& infoLabel, int* action1, int* poste)
    : QLabel(&parent), playerName(playerName.toStdString()), attack(attack), defense(defense), speed(speed), position(position), infoLabel(infoLabel), action1(action1), poste(poste) {
}
